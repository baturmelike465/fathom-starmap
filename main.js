...
var StarmapView = class extends import_obsidian.ItemView {
  ...
  buildData() {
    const app = this.app;
    const files = app.vault.getMarkdownFiles().filter((f) => !/^(_to_delete|copilot|Archive)\//.test(f.path) && !f.path.split("/").some((s) => s.startsWith(".")));
    const byPath = {}, nodes = [];
    ...
    const frame = (now) => {
      ...
      if (nodes.length) {
        ...
        if (flightMode) {
          ...
          ctr.x += (tgt.x - ctr.x) * 0.06;
          ctr.y += (tgt.y - ctr.y) * 0.06;
          ctr.z += (tgt.z - ctr.z) * 0.06;
        }
        if (tour) {
          tour.hold++;
